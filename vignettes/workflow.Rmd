...
imgFeatures$gtvVolNorm <- length(getCoords(gtvImage))/length(getCoords(brainImage))

# Only GTV dependent measurements
imgFeatures$gtvSD <- sd(getMeasurements(gtvImage))

#Distances between points of origins: GTv / Brain
imgFeatures$koordDistCanberraNorm <- dist(
    rbind(getOrigin(gtvImage)*getXFactor(brainImage), 
            getOrigin(brainImage)*getXFactor(brainImage)),
    method="canberra")
```
