...
brainImage@selectedThresholds <- imageanalysisBrain::sampleData(brainImage@measurements, iterat=10000)
```
...
subclassesGTV <- imageanalysisBrain::getSubclasses(gtvImage, brainImage@selectedThresholds)

subclassesBrain <- imageanalysisBrain::getSubclasses(brainImage, brainImage@selectedThresholds)
```
...
## ... and add GTV slice 
imageanalysisBrain::plotZSlice(gtvImage, 10, dim=dim, add=T, col=gplots::bluered(1024)) 
```

Showing only the identified subclass is just as easy (Brain):

```{r}
##Plot first brain subclass
