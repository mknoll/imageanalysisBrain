...
for (i in 1:length(samples)) {
    ...
    if (F) {
        ...
        gtv <- data.frame(fread(gtvFile, sep=",", header=T))
        gtv$COORD <- enc(gtv$x, gtv$y, gtv$z)
            ...
            gtvFile <- system.file("extdata", 
                                    paste(
                                        ...
                                        sep=""),
                                    package = "imageanalysisBrain")
            gtv <- read.table(gzfile(gtvFile), sep=",", header=T)
            gtv$x <- decX(gtv$COORD)
            gtv$y <- decY(gtv$COORD)
            gtv$z <- decZ(gtv$COORD)
            gtv <- gtv[,c(3:5,1:2)]
            }
    ...
    if (F) {
        brainFile <- system.file("extdata", 
                                    paste(
                                        ...
                                        sep=""), 
                                    package = "imageanalysisBrain")
        brain <- data.frame(fread(brainFile, sep=",", header=T))
        brain$COORD <- enc(brain$x, brain$y, brain$z)
            ...
            brainFile <- system.file("extdata", 
                                        paste(
                                            ...
                                            sep=""), 
                                        package = "imageanalysisBrain")
            brain <- read.table(gzfile(brainFile), sep=",", header=T)
            brain$x <- decX(brain$COORD)
            brain$y <- decY(brain$COORD)
            brain$z <- decZ(brain$COORD)
            brain <- brain[,c(3:5,1:2)]
            }
    
    ## calculate x scaling factor 
    xvals <- getXRange(brain)
    ...
    allData[[i]] <- list(name=samples[i],
                        gtv=gtv,
                        brain=brain,
                        gtvZero=getZeroKoord(gtv, draw = F),
                        brainZero=getZeroKoord(brain, draw = F),
                        factor=xfactor
                        )
    }
```
...
for (i in 1:length(allData)) {
    ...
    allBrain[[i]] <- sampleData(allData[[i]]$brain[,4], iterat=5000)
    }

##extract tissue class cutoffsC
print("Collect cutoffs ...")
collectedCutoffs <- determineTissueClasses(allBrain)
```
...
z <- median(data$z)
## Plot brain
plotZSlice(data, z, minx=minx, maxx=maxx, miny=miny, maxy=maxy)
## Add Subclass within GTV
plotZSlice(subclasses[[2]], z, col=greenred(100), add=T, minx=minx, 
